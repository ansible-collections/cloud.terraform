...
options:
  ...
  include_providers:
    description:
      - A list of providers short names to include.
      - Any instances from one of the providers will be included in the inventory.
    type: list
    elements: str
    default: []
    choices:
  ...
  exclude_providers:
    description:
      - A list of providers short names to exclude.
      - Any instances from one of the providers will be excluded from the inventory.
    type: list
    elements: str
    default: []
    choices:
...
def create_providers_list(
    include_providers: List[str], exclude_providers: List[str]
) -> List[TerraformProviderInstance]:
    providers = []
    for name, instance in ProvidersMapping.items():
        if exclude_providers and name in exclude_providers:
            continue
        if not include_providers or (include_providers and name in include_providers):
...
class InventoryModule(TerraformInventoryPluginBase, Constructable):  # type: ignore  # mypy ignore
    ...
    def parse(self, inventory, loader, path, cache=False):  # type: ignore  # mypy ignore
        ...
        exclude_providers = cfg.get("exclude_providers", [])
